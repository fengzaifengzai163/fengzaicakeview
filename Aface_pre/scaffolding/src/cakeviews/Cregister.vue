<template>
  <div>
    ...
    <mt-header title="学前端,到学问">
      <router-link to="/chome" slot="left">
        <mt-button icon="back"></mt-button>
      </router-link>
      <router-link slot="right" to="/ctestlogin">登录</router-link>
    </mt-header>
    <!-- 顶部导航结束 -->
    <!-- 表单区域开始 -->
    <div>
  ...
  methods: {
    ...
    handle(){
      ...
      if(this.checkUsername() && this.checkPassword() && this.checkConpassword()){
        ...
        this.axios.post('/cregister',this.qs.stringify(object)).then(res=>{
          if(res.data.code == 1){
            this.$router.push('/chome');
          }else{
            //弹出提示框
            this.$messagebox("提示信息",res.data.message);
          }
        });
      }
    },
